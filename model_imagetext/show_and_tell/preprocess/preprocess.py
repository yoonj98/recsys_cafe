...
#    for i, img in enumerate(imgs.values()):
    imgs['processed_tokens'] = []
    for j, s in enumerate(imgs.values()):
        txt = str(s).lower().translate(str.maketrans("", "", string.punctuation)).strip().split()
        imgs['processed_tokens'].append(txt)
        #if i < 10 and j == 0:
        print(txt)
...
def main(params):
    ...
    rand_shuffle=list(imgs['caption'].items())  # shuffle the order
    shuffle(rand_shuffle)
    rand_shuffle=dict(rand_shuffle)
    # tokenization and preprocessing
    prepro_captions(rand_shuffle)

    # create the vocab
    vocab = build_vocab(rand_shuffle, params)
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_json', default='/home/wondo94/0conf/20210520 image captioning/radcap_project-master/data_preprocessing/caption.json', help='input json file to process into hdf5')
    ...
    parser.add_argument('--images_root', default='/home/wondo94/0conf/20210520 image captioning/radcap_project-master/data/Images/',
